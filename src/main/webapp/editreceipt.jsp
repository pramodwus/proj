              ...
              <div class="col-md-12">
                          ...
                          <div class="col-sm-2" style="width: 15%;">
                            ...
                            <div class="col-sm-2" style="width: 15%;">
							 ...
							 <div class="col-sm-1" style="width: 13%;">
                                    <input type="text" class="form-control disabledboxes"  id="remark${'$'}{x}"  value=""  >
                                </div>
                 ...
                 <tr >
                    ...
                    <th class="col-sm-1" style="text-align:center;width: 14%;">Remarks</th>
                     ...
                     <input type="hidden" id="category" value="${testdetail[0].testCategory}">
                         <c:forEach items="${testdetail}" var="testDetail" varStatus='loop'>
                        <div class="form-group">
                 			<input type="hidden" id="testdetailid${loop.index}" value="${testDetail.id}">
                 			<input type="hidden" id="cat${loop.index}" value="${testDetail.testCategory}">
                 			<input type="hidden" id="test${loop.index}" value="${testDetail.testName}">
                         <div class="col-sm-2" style="width: 15%;">
                            ...
                            <div class="col-sm-2" style="width: 15%;">
                              ...
                              <div class="col-sm-1" style="width: 9%;">
                               ...
                               <%--   <input type="hidden" class="form-control disabledboxes" id="totaln${loop.index}" value="${testDetail.totalAmount}"> --%>
                                 ...
                                 <div class="col-sm-1" style="width: 13%;">
                                    <input type="text" class="form-control disabledboxes"  id="remarkn${loop.index}"  value="${testDetail.remarks}"  >
                                </div>
                               <div class="col-sm-1">
                        ...
                        <div class="form-group">
                         ...
                         <!--  <div class="col-sm-2">
                          ...
                          <div class="col-sm-2">
                            <input type="text" class="form-control disabledboxes" id="overall"  value="${patient.receiptData.overallRs}" disabled>
                          </div>
                        </div>
                        </table>
                        </fieldset>
...
$(document).ready(function() {
	$('#totalamt').trigger('click');
	 $('#totalamt').hide();
	 debugger;
	 var size='${testdetail.size()}';
	 
		var role='${roles}';
		if(role=='frontdesksonography'){
			var data=['Ultrasound Only Screening','Ultrasound - With Print'];
		}
		if(role=='frontdeskxray'){
			var data=['X-Ray'];
		}
		if(role=='frontdesklabassistant'){
			var category=$('#category').val();
			 if(category.includes('Ultrasound')){
				var data=['Ultrasound Only Screening','Ultrasound - With Print'];
			 }
			 if(category.includes('X-Ray')){
					var data=['X-Ray'];
				 }
		}
		for(i=0;i<size;i++){
			...
			for(j=0;j<data.length;j++){
				 debugger;
		            $('#testCategoryn'+i).append(
	                    $('<option></option>').val(data[j]).html(data[j])
	                );
	                if($('#cat'+i).val()==data[j]){
	                	$("#testCategoryn"+i).val(data[j]); 
	                	newcall(i);
	                }      
			}
		}
	    ...
	    $('#save').click(function(e){
		    ...
		    a.each(function(i,k){
			    ...
			    var data={
						...
						"remarks":$(k).find('#remark'+loop).val(),
						"reciptId":$('#receiptId').val()
						};
				tests.push(data);
				loop++;
	         ...
	         url: 'savedetail',
	            ...
	            success: function(response){
		            if(response==""){
	            		...
	            		alert("Receipt Updated Sucessfully");
		 ...
		 if(parseInt($('#totaln'+id).val())<=parseInt($('#testAmountn'+id).val()) || $('#totaln'+id).val()==''){
			...
			if($('#totaln'+id).val()==''){
				...
				disamount=0;
	 			 ...
	 			 disamount=$('#totaln'+id).val();
			 var docpercent=$('#docpercentn'+id).val();
			  var dcharge=$('#doctorChargesn'+id).val();
			 // $('#doctorChargesn'+id).val(dcharge);
  	            ...
  	            success: function(response){
  	               // $('#specificTestn'+id).empty().append('<option>Select test</option>');
  		            $.each(response, function(val, text) {
  			           ...
  			           $("#specificTestn"+id).val($('#test'+id).val());
  		            });
  	             },
